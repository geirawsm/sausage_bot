...
class SayModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        comment_out = discord_commands.check_user_channel_role(self.children[0].value)
...
async def on_ready():
    ...
    for guild in config.bot.guilds:
        if guild.name == config.env("DISCORD_GUILD"):
            logger.info(
                I18N.t("main.msg.bot_connected", bot=config.bot.user, server=guild.name)
            )
            break
...
async def language(interaction: discord.Interaction, language: str):
    ...
    await config.bot.change_presence(
        activity=discord.Activity(
            ...
            name=config.env("BOT_WATCHING", default=I18N.t("main.msg.bot_watching")),
        )
    )
    await reload_cogs()
    await interaction.followup.send(
