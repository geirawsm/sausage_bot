...
async def prep_table(table_in, inserts: list = None):
    ...
    try:
        ...
        return None
    if "primary" in table_in and table_in["primary"] is not None:
        _cmd += ", PRIMARY KEY({}".format(table_in["primary"])
        if "autoincrement" in table_in and table_in["autoincrement"] is True:
            _cmd += " AUTOINCREMENT"
        _cmd += ")"
    _cmd += ");"
...
async def add_missing_db_setup(template_info, dict_in: dict = None):
    ...
    async with aiosqlite.connect(db_file) as db:
        row_ids = await db.execute(f"SELECT rowid FROM {table_name}")
        ...
        if setting_in[0] in db_type_checking:
            ...
            if db_type_checking[setting_in[0]] == "str":
                ...
                if db_type_checking[setting_in[0]] == "bool":
                    if type(eval(str(setting_in[1]).capitalize())) is not eval(
                        db_type_checking[setting_in[0]]
                    ):
                        remove_status = False
                    else:
                        remove_status = True
...
async def db_channel_names_to_ids(template_info, id_col, channel_col: str):
    ...
    for copy in row_items_copy:
        changes[channel_col].append((channel_col, copy["channel"], copy["channel_new"]))
...
async def db_single_channel_name_to_id(
    ...
    if not re.match(r"(\d+)", channel_in) and len(channel_in) > 0:
        ...
        logger.debug("Channel `{}` is an id and is ok".format(channel_in))
        return True
    else:
        logger.error("Unexpected error")
        return None
    ...
    if cog_name == "roles":
        ...
        if len(settings_file) > 0:
            if "unique_role" in settings_file:
                ...
                logger.debug(f"Got this for `settings_inserts`:\n{settings_inserts}")
            for _msg in settings_file["reaction_messages"]:
                ...
                for react in reacts:
                    reactions_inserts.append((str(__msg["id"]), react[0], react[1]))
        return {
        ...
        for dilemma in dilemmas_file:
            dilemmas_inserts.append((str(uuid4()), dilemmas_file[dilemma]))
        ...
        for quote in quote_file:
            quotes_inserts.append(
                (
                    str(uuid4()),
                    quote_file[quote]["quote"],
                    await get_dt(
                        format="ISO8601",
                        dt=re.sub(r"[\(\)]+", "", quote_file[quote]["datetime"]),
                    ),
                )
            )
            ...
            for insert in stats_settings_inserts:
                if insert[0] in stats_file:
                    ...
                    if isinstance(under_inserts, list):
                        ...
                        stats_inserts.append(
                            (insert[0], stats_file[insert[0]], insert[2], insert[3])
                        )
                else:
                    stats_inserts.append((insert[0], insert[1], insert[2], insert[3]))
            ...
            if (len(filter_allow) + len(filter_deny)) > 0:
                if len(filter_allow) > 0:
                    for line in filter_allow:
                        rss_filter_inserts.append((_uuid, "allow", filter_allow[line]))
                if len(filter_deny) > 0:
                    for line in filter_deny:
                        rss_filter_inserts.append((_uuid, "deny", line))
        ...
        for feed in yt_file:
            ...
            if (len(filter_allow) + len(filter_deny)) > 0:
                if len(filter_allow) > 0:
                    for line in filter_allow:
                        yt_filter_inserts.append((_uuid, "allow", filter_allow[line]))
                if len(filter_deny) > 0:
                    for line in filter_deny:
                        yt_filter_inserts.append((_uuid, "deny", line))
        for feed in yt_logs_file:
            if feed in yt_logs_index:
                for link in yt_logs_file[feed]:
                    yt_logs_inserts.append(
                        (yt_logs_index[feed], link, str(await get_dt(format="ISO8601")))
                    )
        return {
            "feeds": yt_inserts,
    ...
    if args.not_write_database:
        ...
        try:
            async with aiosqlite.connect(db_file) as db:
                ...
                await db.commit()
                logger.debug("Changed {} rows".format(db.total_changes))
...
async def insert_many_some(template_info, rows: tuple = None, inserts: list = None):
    ...
    logger.debug(f"Got `inserts`: {type(inserts)} {len(inserts)}:\n{pformat(inserts)}")
    input_singles = False
    input_multiples = False
    ...
    if isinstance(updates, dict):
        ...
        for update in updates:
            ...
            for _item in updates[update]:
                _cmd += " WHEN {} = '{}' THEN '{}'".format(_item[0], _item[1], _item[2])
...
async def calculate_average_rating_from_db(show_uuid, episode_uuid, template_info):
    ...
    if len(all_ratings) <= 0:
        logger.debug(
            f"No ratings found for show_uuid: {show_uuid}, episode_uuid: {episode_uuid}"
        )
        return None
