    ...
    if any(marker in str(date_in) for marker in ["Z", "T", "+"]):
        ...
        try:
            ...
            if d_len <= 2:
                ...
                return pendulum.from_format(date_in, "HHmm")
            elif d_len == 3:
                # Expecting `DD MM YYYY`, `YYY MM DD` or `DD MM YY`
                if len(d_split[2]) == 4:
                    return pendulum.from_format(date_in, "DD MM YYYY")
                elif len(d_split[0]) == 4:
                    return pendulum.from_format(date_in, "YYYY MM DD")
                elif all(len(timeunit) == 2 for timeunit in d_split):
                    # We have to assume that this is DD MM YY
                    return pendulum.from_format(date_in, "DD MM YY")
            elif d_len == 4:
                ...
                # is the time with a missing separator
                    ...
                    return pendulum.from_format(date_in, "DD MM YYYY HH mm")
            elif d_len == 5:
                if len(d_split[2]) == 4:
                    return pendulum.from_format(date_in, "DD MM YYYY HH mm")
                elif len(d_split[0]) == 4:
                    return pendulum.from_format(date_in, "YYYY MM DD HH mm")
                elif len(d_split[2]) == 2:
                    return pendulum.from_format(date_in, "DD MM YY HH mm")
                elif len(d_split[0]) == 2:
                    return pendulum.from_format(date_in, "YY MM DD HH mm")
            elif d_len == 6:
                if all(len(timeunit) == 2 for timeunit in d_split):
                    d = d_split
...
def change_dt(pendulum_object_in, change=None, count=None, unit=None):
    """
