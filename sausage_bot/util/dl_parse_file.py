...
async def get_nifs_file():
    ...
    match_json = await net_io.get_link(url=base_url.format(_id))
    match_json = json.loads(match_json)
    file_io.write_json(envs.test_nifs_json_good, match_json)
...
async def get_vglive_file():
    ...
    match_json = json.loads(match_json)
    file_io.write_json(envs.test_vglive_json_good, match_json)
    # Get tv info
    ...
    file_io.write_json(envs.test_vglive_tv_json_good, tv_json)


async def get_tv2livesport_file():
    tv2livesport_in = (
        "https://www.tv2.no/livesport/fotball/kamper/"
    ...
    match_json = await net_io.get_link(url=base_url.format(_id))
    match_json = json.loads(match_json)
    file_io.write_json(envs.test_tv2livesport_json_good, match_json)


async def write_generic_rss_to_file(url_in, filename, force=False):
    if (
        (file_io.file_age(envs.TESTPARSE_DIR / filename) > 60 * 60 * 24)
        or file_io.file_exist(envs.TESTPARSE_DIR / filename) is False
        or force
    ):
        feed_info = await net_io.get_link(url=url_in)
        soup = BeautifulSoup(feed_info, features="xml")
        file_io.write_file(envs.TESTPARSE_DIR / filename, soup)
    else:
