...
def url_hostname_matches(url_in, domain):
    """
    Check whether the hostname in `url_in` is `domain` or a subdomain of it.

    Substring checks like `"youtube.com" in url` are fooled by urls where
    the domain shows up outside the hostname, like
    `https://evil.com/youtube.com` or `https://notyoutube.com.evil.net`.

    `url_in`:   The url to check
...
async def fetch_random_user_agent():
    ...
    if (
        ...
        response = requests.get(envs.scrapeops_url.format(config.SCRAPEOPS_API_KEY))
...
async def check_for_new_spotify_podcast_episodes(guild):
    ...
    for feed in spotify_feeds:
        ...
        checklist[pod_id] = {
            "name": feed["feed_name"],
            "num_episodes_old": feed["num_episodes"]
            if isinstance(feed["num_episodes"], int)
            else 0,
...
async def get_other_podcast_links(req, url, uuid, num_items=None, guild=None):
    ...
    items_out = {"filters": filters_db, "items": [], "log": log_db}
    feed_name, feed_description, feed_img = get_channel_info(soup)
    items_info = {
        "feed_name": feed_name,
    ...
    logger.debug("Processing episodes")
    # Gets podcast feed
    podcast_status, _ratio, _signals = is_podcast_feed(soup)
    if podcast_status:
        ...
        try:
            for item in all_items:
                ...
                if isinstance(normal_link, bs4_element.Tag) and "https://" in str(
                    normal_link.next
                ):
                    ...
                    temp_info["link"] = (
                        normal_link.text
                        if hasattr(normal_link, "text")
                        else normal_link
                    )
                logger.debug("link is {}".format(temp_info["link"]))
...
async def make_event_start_stop(date, time=None):
    ...
    try:
        ...
        # the match
        start_event = datetime_handling.change_dt(start_dt, "remove", 30, "minutes")
        ...
        rel_start = discord.utils.format_dt(datetime.fromtimestamp(start_epoch), "R")
...
async def parse_nifs(url_in=None, mock_in=None):
    ...
    if url_in:
        ...
        match_json = await get_link(url_in=base_url.format(_id))
...
async def parse_vglive(url_in=None, mock_in=None, mock_in_tv=None, guild=None):
    ...
    if mock_in and mock_in_tv:
        tv_json = await get_link(mock_file=mock_in_tv)
    else:
    ...
    logger.debug(
        "Channels ({}): {}".format(len(tv_json["tvChannels"]), tv_json["tvChannels"])
    )
...
async def parse_tv2livesport(url_in=None, mock_in=None):
    ...
    in an API call
    """
    base_url = "https://livesport-api.alpha.tv2.no/v3/football/matches/{}/result"
    # Get info relevant for the event
    if url_in:
        _id = re.match(r".*tv2.no/livesport/.*/kamper/.*/([a-f0-9\-]+)", url_in).group(
            1
        )
...
def clean_pod_description(desc_in):
    ...
    if "hosted on acast. see" in desc_in.lower():
        desc_in = desc_in.split("Hosted on Acast")[0]
    if "nrss.deno" in desc_in.lower():
        desc_in = desc_in.replace("⬇️NRSS er avhengig av din Vipps-støtte⬇️", "")
        desc_in = re.sub(
            r"Takk for at du bruker NRSS.*Se mer på https://nrss.deno.dev/", "", desc_in
        )
...
async def get_page_hash(url, debug=False):
    ...
    if desc is None and url_hostname_matches(url, "open.spotify.com"):
        ...
        try:
            ...
            pass
    dt = await datetime_handling.get_dt(format="revdatetimefull", sep="-")
    if desc is None:
