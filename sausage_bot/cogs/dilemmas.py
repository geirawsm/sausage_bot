...
class Dilemmas(commands.Cog):
    ...
    async def dilemmas(self, interaction: discord.Interaction) -> None:
        ...
        no_of_dilemmas = await db_helper.get_output(
            template_info=envs.dilemmas_db_schema, select=("id")
        )
        ...
        await db_helper.insert_many_all(
            envs.dilemmas_db_log_schema, [(random_dilemma[0][0], dilemma_post.id)]
        )
        return
    ...
    async def dilemmas_add(
        ...
        await interaction.response.defer()
        await db_helper.insert_many_all(
            envs.dilemmas_db_schema, [(str(uuid.uuid4()), dilemmas_in)]
        )
        await interaction.followup.send(
...
async def setup(bot):
    ...
    if not file_io.file_exist(envs.dilemmas_db_schema["db_file"]):
        ...
        await db_helper.prep_table(envs.dilemmas_db_log_schema)
    else:
