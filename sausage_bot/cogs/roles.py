...
# Autocomplete choices pack more than one field into a single value. A uuid
# holds `-` itself, so the fields are joined by something a uuid never holds.
AUTOCOMPLETE_SEP = ";"
    ...
    def __init__(self, permissions_in=None):
        def prep_dropdown(perm_name, permissions_in: dict = None):
            ...
            for perm in envs.SELECT_PERMISSIONS[perm_name]:
                ...
                if isinstance(permissions_in, (dict, list)) and perm in permissions_in:
                    list_out.append(
                        discord.SelectOption(
                            label=perm, description=_desc, value=perm, default=True
                        )
                    )
                else:
                    list_out.append(
                        discord.SelectOption(label=perm, description=_desc, value=perm)
                    )
            return list_out
        ...
        general_perms = prep_dropdown("general", self.permissions_in)
        text_perms = prep_dropdown("text", self.permissions_in)
        voice_perms = prep_dropdown("voice", self.permissions_in)
        ...
        text_dropdown = DropdownPermissions(
            I18N.t("roles.views.perms.drop_text"), self.permissions_out, text_perms
        )
...
async def settings_autocomplete(
    ...
    return [
        discord.app_commands.Choice(
            name="{}. {} = {} ({})".format(
                setting["rowid"], setting["setting"], setting["name"], setting["value"]
            ),
...
async def emojis_autocomplete(
    ...
    for emoji in _emojis:
        _emojis_list.append((emoji.name, emoji.id))
    logger.debug(f"_emojis_list: {_emojis_list}")
    return [
        discord.app_commands.Choice(
            name=f"{emoji[0]} ({emoji[1]})", value=str(emoji[1])
        )
        for emoji in _emojis_list
        if current.lower() in emoji[0].lower()
    ][:25]


async def get_msg_info(msg_in, guild_id):
    """
    Get uuid, msg id, channel and message name from database based on
    uuid, msg id or msg name.

    `uuid` is what ties a reaction message to its reaction roles, so that
    is what callers normally hand in. msg id and name are still accepted
    because those are what a human has at hand.
    """
    msg_in = str(msg_in)
    logger.debug(f"Got `msg_in`: {msg_in}")
    if re.match(UUID_REGEX, msg_in):
        logger.debug("Got uuid input")
        where_in = "uuid"
    elif re.match(r"^[0-9]+$", msg_in):
    ...
    db_message = await db_helper.get_output(
        ...
        where=[(where_in, msg_in)],
        select=("uuid", "msg_id", "channel", "name"),
        single=True,
        guild_id=guild_id,
    )
    logger.debug(f"db_message: {db_message}")
    if not db_message:
        logger.error(f"Could not find reaction message `{msg_in}` in database")
        return None
    return {
        "uuid": db_message["uuid"],
        "id": int(db_message["msg_id"]) if db_message["msg_id"] else None,
...
async def sync_reaction_message_from_settings(
    msg_uuid, sort: bool = False, guild: discord.Guild = None
):
    # Assert that the reaction message exist on discord
    msg_info = await get_msg_info(msg_uuid, guild.id)
    _guild = guild
    logger.debug(f"msg_info:\n{pformat(msg_info)}")
    if msg_info is None:
        return
    msg_uuid = msg_info["uuid"]
    msg_id = msg_info["id"]
    ...
    if msg_obj is None:
        # If the message has been deleted, it needs to be recreated, and
        # msg_id in the messages table must be updated. The reaction roles
        # hang off the uuid and are left alone.
        ...
        # Update database with correct message ID
        logger.debug("Replace old id ({}) with new ({})".format(msg_id, msg_obj.id))
        msg_id = msg_obj.id
    ...
    for reaction in reactions_out:
        ...
        logger.debug(
            "Trying to add emoji: `{}` ({})".format(_emoji_id, type(_emoji_id))
        )
        try:
...
def tabulate_emoji(dict_in):
    content = {
        "emoji": {"length": 7, "header": I18N.t("roles.emoji_headers.emoji")},
        "name": {"length": 0, "header": I18N.t("roles.emoji_headers.name")},
        "id": {"length": 20, "header": I18N.t("roles.emoji_headers.id")},
        "animated": {"length": 11, "header": I18N.t("roles.emoji_headers.animated")},
        "managed": {"length": 17, "header": I18N.t("roles.emoji_headers.managed")},
    }
...
def tabulate_roles(dict_in):
    content = {
        "emoji": {"length": 7, "header": I18N.t("roles.roles_headers.emoji")},
        "name": {"length": 0, "header": I18N.t("roles.roles_headers.name")},
        "id": {"length": 20, "header": I18N.t("roles.roles_headers.id")},
        "members": {"length": 8, "header": I18N.t("roles.roles_headers.members")},
        "managed": {"length": 17, "header": I18N.t("roles.roles_headers.managed")},
    }
    for dict_item in dict_in:
        for item in dict_in[dict_item]:
            if len(str(item)) > content[dict_item]["length"]:
...
def tabulate_emojis_and_roles(dict_in):
    content = {
        ...
        "emoji_name": {
            "length": len(str(I18N.t("roles.tabulate_emojis_and_roles.emoji_name"))),
        ...
        "role_name": {
            "length": len(str(I18N.t("roles.tabulate_emojis_and_roles.role_name"))) + 1,
    ...
    while counter <= len(dict_in["emoji_id"]) - 1:
        line_out = "{} `{:{}} {:{}} {:{}} {:{}}`".format(
            "<:{}:{}>".format(
                dict_in["emoji_name"][counter], dict_in["emoji_id"][counter]
            ),
            dict_in["emoji_name"][counter],
            content["emoji_name"]["length"],
            dict_in["emoji_id"][counter],
            content["emoji_id"]["length"],
            dict_in["role_name"][counter],
            content["role_name"]["length"],
            dict_in["role_id"][counter],
            content["role_id"]["length"],
        )
        if len(line_out) == 0:
            line_out = temp_out
...
async def reaction_msgs_autocomplete(
    ...
    db_reactions = await db_helper.get_output(
        template_info=envs.roles_db_msgs_schema,
        select=("uuid", "msg_id", "name"),
        order_by=[("name", "ASC")],
        guild_id=interaction.guild.id,
    )
    logger.debug(f"db_reactions: {db_reactions}")
    # The uuid is the only thing handed back - everything else about the
    # message is looked up from it, and it does not go stale the way a
    # msg_id does
    return [
        discord.app_commands.Choice(
            name=str(reaction["name"]),
            value=str(reaction["uuid"]),
        )
...
async def reaction_msgs_roles_autocomplete(
    ...
    db_reactions = await db_helper.get_combined_output(
        ...
        key="uuid",
        select=["name", "channel", "A.uuid", "role", "emoji"],
        guild_id=interaction.guild.id,
    )
    _guild = interaction.guild
    ...
    return [
        discord.app_commands.Choice(
            ...
            # A dirty little hack here, returning uuid, role and emoji as
            # a combined string
            value=AUTOCOMPLETE_SEP.join(
                [
                    str(reaction["uuid"]),
                    str(reaction["role"]),
                    str(reaction["emoji"]),
        ...
        for reaction in db_reactions
        if current.lower() in "{}-{}".format(reaction["name"], reaction["role"]).lower()
    ][:25]
...
class ReactionEditModal(discord.ui.Modal):
    def __init__(self, title_in=None, reaction_header_in=None, reaction_text_in=None):
    ...
    async def on_submit(
        ...
        await interaction.response.send_message(
            I18N.t("roles.modals.reaction_edit.confirm"), ephemeral=True
        )

    async def on_error(self, interaction: discord.Interaction, error):
        logger.debug(f"Error when editing reaction message: {error}")
        await interaction.response.send_message(
            I18N.t("roles.modals.reaction_edit.error", error=error), ephemeral=True
        )
...
class Autoroles(commands.Cog):
    ...
    roles_group = discord.app_commands.Group(
        name="roles", description=locale_str(I18N.t("roles.group.roles"))
    )

    roles_reaction_group = discord.app_commands.Group(
        name="reaction",
        description=locale_str(I18N.t("roles.group.reaction")),
        parent=roles_group,
    )

    roles_reaction_add_group = discord.app_commands.Group(
        name="reaction_add",
        description=locale_str(I18N.t("roles.group.add_reaction")),
        parent=roles_group,
    )

    roles_reaction_remove_group = discord.app_commands.Group(
        name="reaction_remove",
        description=locale_str(I18N.t("roles.group.remove_reaction")),
        parent=roles_group,
    )

    roles_reaction_move_group = discord.app_commands.Group(
        name="reaction_move",
        description=locale_str(I18N.t("roles.group.move_reaction_role")),
        parent=roles_group,
    )

    roles_reaction_edit_group = discord.app_commands.Group(
        name="reaction_edit",
        description=locale_str(I18N.t("roles.group.edit_reaction")),
        parent=roles_group,
    )

    roles_settings_group = discord.app_commands.Group(
        name="settings",
        description=locale_str(I18N.t("roles.group.settings")),
        parent=roles_group,
    )
    ...
    async def role_info(
        ...
        embed.add_field(name=I18N.t("common.color"), value=role_in.color, inline=True)
        if role_in.is_bot_managed():
            embed.add_field(
                name=I18N.t("roles.embed.auto_managed.name"),
                value=I18N.t(
                    "roles.embed.auto_managed.value_confirm", name=role_in.name
                ),
                inline=True,
            )
            ...
            embed.add_field(
                name=I18N.t("roles.embed.auto_managed.name"),
                value=I18N.t("common.literal_yes_no.lit_no"),
                inline=True,
            )
        embed.add_field(
            name=I18N.t("roles.embed.hoist.name"),
            value=I18N.t("common.literal_yes_no.lit_yes")
            if role_in.hoist
            else I18N.t("common.literal_yes_no.lit_no"),
            inline=True,
        )
        embed.add_field(
            name=I18N.t("roles.embed.members"), value=len(role_in.members), inline=True
        )
        permissions = ", ".join(
            [
                permission
                for permission, value in iter(role_in.permissions)
                if value is True
            ]
        )
        embed.add_field(
    ...
    async def roles_list(
        self,
        interaction: discord.Interaction,
        type: typing.Literal[I18N.t("common.roles"), I18N.t("common.emojis")],
        sort: typing.Literal[I18N.t("common.name"), I18N.t("common.id")],
        public: typing.Literal[
            I18N.t("common.literal_yes_no.lit_yes"),
            I18N.t("common.literal_yes_no.lit_no"),
        ] = None,
        ...
        async def roles_list_roles():
            ...
            if sort == I18N.t("common.name"):
                _roles = tuple(sorted(_guild.roles, key=lambda role: role.name.lower()))
            elif sort == I18N.t("common.id"):
                _roles = tuple(sorted(_guild.roles, key=lambda role: role.id))
            for role in _roles:
        ...
        async def roles_list_emojis():
            ...
            if sort == I18N.t("common.name"):
                _emojis = tuple(
                    sorted(_guild.emojis, key=lambda emoji: emoji.name.lower())
                )
            elif sort == I18N.t("common.id"):
                _emojis = tuple(sorted(_guild.emojis, key=lambda emoji: emoji.id))
            for emoji in _emojis:
                tabulate_dict["emoji"].append(f"<:{emoji.name}:{emoji.id}>")
    ...
    async def add_role(
        ...
        try:
            ...
            await interaction.followup.send(
                I18N.t("roles.commands.add_role.msg_confirm"), ephemeral=_ephemeral
            )
    ...
    @describe(role_name=I18N.t("roles.commands.remove_role.desc.role_name"))
    ...
    async def edit_role(
        ...
        if permissions:
            ...
            perm_view = PermissionsView(permissions_in=perms_in)
            await interaction.followup.send(
                I18N.t("roles.commands.edit_role.change_perms"), view=perm_view
            )
            await perm_view.wait()
            perms_out = perm_view.permissions_out
            logger.debug(f"`perm_view.permissions_out`: {perm_view.permissions_out}")
            new_perms = ""
            if len(perms_out) > 0:
                new_perms = ", ".join(f"{perm}=True" for perm in perms_out)
            # Edit role in guild
            await role_name.edit(permissions=eval(f"discord.Permissions({new_perms})"))
        ...
        if len(changes) > 0:
            changes_out = "{}:".format(
                I18N.t("roles.commands.edit_role.changes_out", role_name=role_name.name)
            )
            for change in changes:
                changes_out += change
            await interaction.followup.send(changes_out)
        else:
            await interaction.followup.send(
                I18N.t("roles.commands.edit_role.no_changes", role=role_name.name)
            )
        return
    ...
    async def add_emoji(
        ...
        try:
            ...
            await interaction.followup.send(
                I18N.t("roles.commands.add_emoji.confirm_msg", emoji_name=emoji_name)
            )
        except discord.errors.Forbidden as e:
            logger.error(f"Could not add emoji - forbidden: {e}")
            await interaction.followup.send(
                I18N.t("roles.commands.add_emoji.msg_error", error=e.text)
            )
            return
        except ValueError as e:
    ...
    @describe(emoji=I18N.t("roles.commands.remove_emoji.desc.emoji_name"))
    async def remove_emoji(self, interaction: discord.Interaction, emoji: discord.Role):
        ...
        try:
            ...
            await interaction.followup.send(
                I18N.t("roles.commands.remove_emoji.confirm_msg", emoji_name=emoji_name)
            )
            return
        except discord.errors.Forbidden as e:
            logger.error(f"Could not remove emoji - forbidden: {e}")
            await interaction.followup.send(
                I18N.t("roles.commands.remove_emoji.error_msg", error=e.text)
            )
            return
        except ValueError as e:
            logger.error(f"Could not add emoji - ValueError: {e}")
    ...
    async def edit_emoji(
        ...
        if new_name:
            ...
            await emoji_obj.edit(name=new_name, reason=reason if not None else "")
            logger.debug("Changed name")
        if roles:
            i18n_roles = I18N.t("roles.changelist.roles")
            changes.append(f"\n- {i18n_roles}: `{emoji_obj.roles}` -> `{roles}`")
            await emoji_obj.edit(roles=roles, reason=reason if not None else "")
            logger.debug("Changed allowed roles")

        if len(changes) > 0:
            changes_out = "{}:".format(
                I18N.t(
                    "roles.commands.edit_emoji.changes_out", emoji_name=emoji_obj.name
                )
            )
            for change in changes:
                changes_out += change
            ...
            await interaction.followup.send(
                I18N.t("roles.commands.edit_emoji.no_changes", emoji=emoji_obj.name)
            )
        return
    ...
    async def emoji_info(
        ...
        embed.add_field(name=I18N.t("common.name"), value=emoji_obj.name, inline=True)
        embed.add_field(
            name=I18N.t("common.color"), value=f"#{emoji_color}", inline=True
        )
        if emoji_obj.managed:
            embed.add_field(
                name=I18N.t("roles.embed.auto_managed.name"),
                value=I18N.t(
                    "roles.embed.auto_managed.value_confirm", name=emoji_obj.name
                ),
                inline=True,
            )
        else:
    ...
    @roles_reaction_group.command(
        name="list", description=locale_str(I18N.t("roles.commands.react_list.cmd"))
    )
    async def list_reactions(
        self, interaction: discord.Interaction, reaction_msg: str = None
    ):
        await interaction.response.defer(ephemeral=True)
        if reaction_msg:
            msg_uuid = reaction_msg
            db_reactions = await db_helper.get_combined_output(
                envs.roles_db_msgs_schema,
                envs.roles_db_roles_schema,
                key="uuid",
                ...
                where=[("A.uuid", msg_uuid)],
                guild_id=interaction.guild.id,
            )
            logger.debug(f"db_reactions:\n{pformat(db_reactions)}")
            if db_reactions is None or len(db_reactions) <= 0:
                await interaction.followup.send(
                    I18N.t(
                        "roles.commands.react_list.msg_error",
                        reaction_msg=msg_uuid,
                    )
                )
                return
            tabulate_dict = {
            ...
            for reaction in db_reactions:
                ...
                if re.match(r"\d{19,22}", reaction["emoji"]):
                    ...
                    emoji_name = I18N.t("roles.commands.react_list.emoji_error")
            ...
            await interaction.followup.send(
                "{}: `{}`\n{}: `{}`\n{}: `{}`\n{}: `{}`\n{}: `{}`\n\n{}".format(
                    ...
                    db_reactions[0]["channel"],
                    I18N.t("common.message_id"),
                    db_reactions[0]["msg_id"],
                    I18N.t("common.header"),
                    _header if _header is not None else " ",
                    I18N.t("common.text"),
                    db_reactions[0]["content"],
            ...
            sorted_reacts = await db_helper.get_combined_output(
                template_info_1=envs.roles_db_msgs_schema,
                template_info_2=envs.roles_db_roles_schema,
                select=["name", "channel", "msg_order", "A.msg_id", "content"],
                key="uuid",
                group_by="A.uuid",
            ...
            if sorted_reacts is None:
                await interaction.followup.send("Ingen meldinger i databasen")
                return
    ...
    @roles_reaction_add_group.command(
        name="message",
        description=locale_str(I18N.t("roles.commands.add_reaction_msg.cmd")),
    )
    @describe(
    ...
    async def add_reaction_message(
        ...
        if len(msg_db) == 1:
            await interaction.followup.send(
                I18N.t(
                    "roles.commands.add_reaction_msg.msg_reaction_already_exist",
                    msg_name=msg_name,
                )
            )
            return
        ...
        if merged_roles_emojis is None:
            await interaction.followup.send(
                I18N.t("roles.commands.add_reaction_msg.msg_roles_emojis_error")
            )
            return
        ...
        # the reaction message lives, no matter how many times the discord
        # message behind it is recreated
        msg_uuid = str(uuid4())
        # Save to DB
        reactions_in = []
        for reac in reactions:
            logger.debug(f"Checking reac {reac}")
            reactions_in.append((msg_uuid, reac[0], reac[1]))
        ...
        await interaction.followup.send(
            I18N.t("roles.commands.add_reaction_msg.msg_confirm"), ephemeral=True
        )
        return
    ...
    @discord.app_commands.autocomplete(msg_info=reaction_msgs_autocomplete)
    async def add_reaction_role(
        ...
        """
        await interaction.response.defer(ephemeral=True)
        msg_db = await get_msg_info(msg_info, interaction.guild.id)
        if msg_db is None:
            await interaction.followup.send(
                I18N.t("roles.commands.remove_msg.msg_error"), ephemeral=True
            )
            return
        msg_uuid = msg_db["uuid"]
        msg_name = msg_db["name"]
    ...
    @roles_reaction_group.command(
        name="sync", description=locale_str(I18N.t("roles.commands.sync.cmd"))
    )
    async def sync_reaction_items(
        ...
        if sync_errors:
            ...
            await interaction.followup.send(
                I18N.t("roles.commands.sync.confirm_sync"), ephemeral=True
            )
        return
    ...
    async def sort_reaction_items(
        ...
        await interaction.response.defer(ephemeral=True)
        # Get message object
        msg_info = await get_msg_info(reaction_msg, interaction.guild.id)
        if msg_info is None:
            await interaction.followup.send(
        ...
        if _msg is None:
            await interaction.followup.send(
                I18N.t("roles.commands.sort.msg_error", reaction_msg=msg_info["name"])
            )
            return
        sync_errors = await sync_reaction_message_from_settings(
            msg_uuid=msg_info["uuid"], sort=True, guild=interaction.guild
        )
        if sync_errors:
            await interaction.followup.send(sync_errors, ephemeral=True)
        else:
            await interaction.followup.send(
                I18N.t("roles.commands.sort.msg_confirm"), ephemeral=True
            )
        return
    ...
    async def remove_reaction_message(
        ...
        if msg_info is None:
            await interaction.followup.send(
                I18N.t("roles.commands.remove_msg.msg_error")
            )
            return
        _msg = await discord_commands.get_message_obj(
            guild=interaction.guild,
        ...
        if _msg is None:
            ...
            return
        # Remove the reaction roles hanging off this message, then the
        # message itself. Both are keyed on the same uuid.
        await db_helper.del_row_by_AND_filter(
            template_info=envs.roles_db_roles_schema,
            where=[("uuid", msg_info["uuid"])],
            guild_id=interaction.guild.id,
        )
        await db_helper.del_row_by_AND_filter(
            template_info=envs.roles_db_msgs_schema,
    ...
    async def edit_reaction_message(
        ...
        db_reactions = await db_helper.get_output(
            template_info=envs.roles_db_msgs_schema,
            where=(("uuid", msg_uuid)),
            select=("uuid", "msg_id", "name", "channel", "header", "content"),
            order_by=[("name", "ASC")],
            single=True,
            guild_id=interaction.guild.id,
        )
        ...
        if not db_reactions:
            await interaction.response.send_message(
                I18N.t("roles.commands.edit_reaction_msg.msg_error"), ephemeral=True
            )
            return
        msg_id = db_reactions["msg_id"]
        ...
        logger.debug(f"`modal_in.reaction_text_out` is {modal_in.reaction_text_out}")
        db_updates = [
    ...
    async def move_reaction_role(
        self,
        interaction: discord.Interaction,
        reaction_role_from: str,
        reaction_message_to: str,
        sort: bool = False,
    ):
        """
        ...
        """
        await interaction.response.defer(ephemeral=True)
        reaction_role_from = reaction_role_from.split(AUTOCOMPLETE_SEP)
        old_msg_uuid = reaction_role_from[0]
        role_id = reaction_role_from[1]
        emoji_id = reaction_role_from[2]
        new_msg_uuid = reaction_message_to
        old_msg_info = await get_msg_info(old_msg_uuid, interaction.guild.id)
        new_msg_info = await get_msg_info(new_msg_uuid, interaction.guild.id)
        if old_msg_info is None or new_msg_info is None:
            await interaction.followup.send(
                I18N.t("roles.commands.remove_msg.msg_error"), ephemeral=True
            )
            return
        old_msg_name = old_msg_info["name"]
        new_msg_name = new_msg_info["name"]
        num_reaction_roles = await db_helper.get_output(
            template_info=envs.roles_db_roles_schema,
            where=("uuid", old_msg_uuid),
            select=("role"),
            guild_id=interaction.guild.id,
        )
        if len(num_reaction_roles) == 1:
            await interaction.followup.send(
                I18N.t(
                    "roles.commands.move_reaction_role.desc.cannot_move_last_reaction"
                ),
                ephemeral=True,
            )
            return
        ...
        await db_helper.insert_many_all(
            envs.roles_db_roles_schema,
            inserts=[(new_msg_uuid, role_id, emoji_id)],
            guild_id=interaction.guild.id,
        )
        # Delete reaction from old message in db
        await db_helper.del_row_by_AND_filter(
            template_info=envs.roles_db_roles_schema,
            where=[("uuid", str(old_msg_uuid)), ("role", str(role_id))],
            guild_id=interaction.guild.id,
        )
        # Sync settings
        await sync_reaction_message_from_settings(
            old_msg_uuid, sort=sort, guild=interaction.guild
        )
        await sync_reaction_message_from_settings(
            new_msg_uuid, sort=sort, guild=interaction.guild
        )
        await interaction.followup.send(
            I18N.t(
                ...
                emoji=emoji_obj,
                role=role_obj,
                old_msg=old_msg_name,
                new_msg=new_msg_name,
            ),
            ephemeral=True,
        )
        return
    ...
    @describe(channel=I18N.t("roles.commands.reorder.desc.channel"))
    async def reorder_reaction_messages(
        ...
        async def update_msg_id(msg_uuid, new_msg):
            # Only the messages table carries the discord message id. The
            # reaction roles are tied to the uuid and need no rewriting.
            await db_helper.update_fields(
                envs.roles_db_msgs_schema,
        ...
        if trigger_reordering:
            ...
            for react_msg in react_msgs:
                ...
                new_reaction_msg = await discord_commands.post_to_channel(
                    ...
                    embed_in={"description": react_msg["content"]},
                )
                # Update msg id in the messages table
                await update_msg_id(
                    msg_uuid=react_msg["uuid"], new_msg=new_reaction_msg.id
                )
                # Delete message
    ...
    async def add_settings(
        ...
        setting: typing.Literal[
            I18N.t("roles.commands.add_settings.literal.setting.unique"),
            I18N.t("roles.commands.add_settings.literal.setting.not_include_in_total"),
        ],
        role: discord.Role,
    ):
        """
        ...
        """
        await interaction.response.defer(ephemeral=True)
        if setting == I18N.t("roles.commands.add_settings.literal.setting.unique"):
            _setting = "unique"
    ...
    @roles_settings_group.command(
        name="remove",
        description=locale_str(I18N.t("roles.commands.remove_settings.cmd")),
    )
    @describe(setting=I18N.t("roles.commands.remove_settings.desc.setting"))
    async def remove_settings(self, interaction: discord.Interaction, setting: str):
        """
    ...
    async def list_settings(self, interaction: discord.Interaction):
        ...
        _settings = tabulate(
            temp_settings_db,
            headers={
                "setting": I18N.t("roles.commands.list_settings.headers.setting"),
    ...
    async def edit_settings(
        self, interaction: discord.Interaction, setting: str, role: discord.Role
    ):
        """
...
async def convert_roles_db_to_uuid(guild):
    """
    Convert this guild's roles tables from `msg_id` to `uuid` if they are
    still on the old layout.

    A reaction message is recreated now and then - it is deleted on
    discord, the channel is reordered, someone runs `/roles reaction
    recreate` - and every recreation hands it a new `msg_id`. Keying
    `roles` on `msg_id` meant every recreation had to rewrite both
    tables, and a missed rewrite left the reactions pointing at a message
    id nobody uses anymore. `uuid` is handed out once, when the message
    is added, and never changes.

    What it does to an old database:

    1. `messages` gets a `uuid` column, and every row without one is
       given a freshly generated uuid4 which is written back to the
       table.
    2. `roles` gets a `uuid` column, filled in from `messages` by
       matching the old `msg_id`, so every existing reaction role
       follows its message over.
    3. Reaction roles left without a uuid are deleted - they point at a
       reaction message that is not in the database anymore.
    4. Both tables are rebuilt in the column order the schemas in `envs`
       state. This matters: `db_helper.insert_many_all` inserts by
       position, and `ALTER TABLE ADD COLUMN` appends, so a `uuid`
       hanging off the end would send every new row into the wrong
       columns. The rebuild also moves the primary key of `messages`
       from `msg_id` to `uuid`.

    Safe to call repeatedly (idempotent): a database that already holds
    the new layout is left alone.
    """
    msgs_schema = envs.roles_db_msgs_schema
    roles_schema = envs.roles_db_roles_schema
    msgs_wanted = [item[0] for item in msgs_schema["items"]]
    roles_wanted = [item[0] for item in roles_schema["items"]]
    msgs_cols = await db_helper.list_cols(msgs_schema, guild_id=guild.id)
    roles_cols = await db_helper.list_cols(roles_schema, guild_id=guild.id)
    logger.debug(f"`msgs_cols`: {msgs_cols} - `roles_cols`: {roles_cols}")
    if len(msgs_cols) == 0:
        logger.debug("No reaction messages table to convert")
        return False
    if msgs_cols == msgs_wanted and roles_cols in ([], roles_wanted):
    ...
    if "uuid" not in msgs_cols:
        await db_helper.add_cols(msgs_schema, ["uuid"], guild_id=guild.id)
    ...
    logger.debug(f"`db_msgs`: {pformat(db_msgs)}")
    # (rowid, msg_id, new uuid) for every message that has none yet
    new_uuids = [
        (msg["rowid"], msg["msg_id"], str(uuid4()))
        for msg in db_msgs
        if not msg["uuid"]
    ]
    if len(new_uuids) > 0:
        logger.info(f"Generating uuid for {len(new_uuids)} reaction message(s)")
        await db_helper.update_fields(
            msgs_schema,
            updates={"uuid": [("rowid", _new[0], _new[2]) for _new in new_uuids]},
            guild_id=guild.id,
        )
    # Every message now has a uuid - map the old msg_id to it
    msg_id_to_uuid = {
        str(_new[1]): _new[2] for _new in new_uuids if _new[1] is not None
    }
    for msg in db_msgs:
        if msg["uuid"] and msg["msg_id"] is not None:
            msg_id_to_uuid[str(msg["msg_id"])] = msg["uuid"]
    logger.debug(f"`msg_id_to_uuid`: {pformat(msg_id_to_uuid)}")

    # 2. Point the reaction roles at the uuid instead of the msg_id
    if len(roles_cols) > 0 and "uuid" not in roles_cols and "msg_id" in roles_cols:
        await db_helper.add_cols(roles_schema, ["uuid"], guild_id=guild.id)
        if len(msg_id_to_uuid) > 0:
            await db_helper.update_fields(
                roles_schema,
                updates={
                    "uuid": [
                        ("msg_id", msg_id, msg_uuid)
                        for msg_id, msg_uuid in msg_id_to_uuid.items()
                    ]
                },
                guild_id=guild.id,
            )
        # 3. Drop reaction roles that have no reaction message left
        ...
        orphans = [_role["rowid"] for _role in db_roles if not _role["uuid"]]
        if len(orphans) > 0:
            logger.error(
    ...
    # 4. Put the columns back in schema order and move the primary key
    if msgs_cols != msgs_wanted:
        await db_helper.rebuild_table_from_schema(msgs_schema, guild_id=guild.id)
    if len(roles_cols) > 0 and roles_cols != roles_wanted:
        await db_helper.rebuild_table_from_schema(roles_schema, guild_id=guild.id)
    return True
...
async def ensure_guild_roles_tables(guild):
    ...
    # The roles tables used to hang off `msg_id`, which changes every time
    # a reaction message is recreated. Move them over to `uuid`.
    await convert_roles_db_to_uuid(guild)
...
async def on_raw_reaction_add(payload):
    ...
    reaction_messages = await db_helper.get_output(
        envs.roles_db_msgs_schema, select=("uuid", "msg_id"), guild_id=_guild.id
    )
    for reaction_message in reaction_messages:
        if str(payload.message_id) == str(reaction_message["msg_id"]):
            ...
            reactions = await db_helper.get_combined_output(
                ...
                key="uuid",
                select=["emoji", "role"],
                where=[("A.uuid", reaction_message["uuid"])],
                guild_id=_guild.id,
            )
            ...
            for reaction in reactions:
                ...
                logger.debug(
                    "Comparing emoji from payload ({}) with emoji from db ({})".format(
                ...
                if str(incoming_emoji) == str(reaction["emoji"]):
                    for _role in _guild.roles:
                        if str(_role.id) in reaction["role"].lower():
                            logger.debug(
                                "Removing role {} from user".format(reaction["role"])
                            )
                            await _guild.get_member(payload.user_id).remove_roles(
                                _role,
                                reason=I18N.t(
                                    "roles.on_raw_reaction_remove.channel_log_confirm"
                                ),
                            )
                            break
    return
...
async def on_member_update(before, after):
    ...
    if unique_role:
        ...
        logger.debug(
            "after.roles: {}".format(", ".join(role.name for role in after.roles))
        )
        if len(after.roles) and all(unique_role == role.id for role in after.roles):
        ...
        if int(_after) <= 0:
            ...
            await after.add_roles(get(_guild.roles, id=int(unique_role)))
        elif int(_after) > 1:
            logger.debug("Length of after.roles is more than 1, removing unique role")
            await after.remove_roles(get(_guild.roles, id=int(unique_role)))
    return
